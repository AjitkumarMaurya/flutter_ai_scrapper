...
class CacheManager {
  ...
  Future<void> initialize({CacheConfig? config}) async {
    ...
    if (_config.persistToDisk && !kIsWeb) {
      try {
        ...
        _cacheFile = File('${cacheDir.path}/flutter_ai_scrapper_cache.json');
        await _loadFromDisk();
      } catch (e) {
        if (kDebugMode) {
  ...
  Future<void> clear() async {
    ...
    if (_config.persistToDisk && _cacheFile != null) {
      try {
        // Replaced wholesale by the two-tier LRU store in Phase 1.4; the
        // synchronous variant is not worth introducing into code on its way out.
  ...
  Future<void> _loadFromDisk() async {
    // See the note in clear(): this file is superseded in Phase 1.4.
    // ignore: avoid_slow_async_io
