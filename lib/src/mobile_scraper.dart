...
class MobileScraper {
  ...
  List<String> queryAll({
    ...
    try {
      ...
      final String pattern = _buildTagPattern(tag, className: className, id: id);

      final RegExp regex = RegExp(pattern, caseSensitive: false, dotAll: true);
      final Iterable<RegExpMatch> matches = regex.allMatches(_htmlContent!);

      for (RegExpMatch match in matches) {
        final String? content = match.group(1);
        if (content != null) {
          // Remove HTML tags from content and clean up whitespace
          final String cleanContent = _cleanHtmlContent(content);
  ...
  String? query({
    ...
    final List<String> results = queryAll(tag: tag, className: className, id: id);
  ...
  List<String> queryWithRegex({
    ...
    try {
      ...
      for (RegExpMatch match in matches) {
        final String? content = match.group(group);
        if (content != null) {
  ...
  String? queryWithRegexFirst({
    ...
    final List<String> results = queryWithRegex(pattern: pattern, group: group);
    return results.isNotEmpty ? results.first : null;
  }
